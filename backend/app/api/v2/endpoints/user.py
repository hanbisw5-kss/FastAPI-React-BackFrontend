...
from app.schemas.user import UserCreate, User_iue, UserUpdate
...
@router.get("/", response_model=List[User_iue])
async def read_users(db: AsyncSession = Depends(get_db)):
    # users = get_users(db)
    return await get_users(db)


@router.post("/", response_model=List[User_iue])
async def create_user_endpoint(user: UserCreate, db: AsyncSession = Depends(get_db)):
    return await create_user(db, user)

@router.get("/{user_id}", response_model=List[User_iue])
...
@router.put("/{user_id}", response_model=List[User_iue])
async def update_user_endpoint(user_id: int, user: UserUpdate, db: AsyncSession = Depends(get_db)):
    return await update_user(db, user_id=user_id, user=user)
