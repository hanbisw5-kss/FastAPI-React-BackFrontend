from sqlalchemy import select, text
from sqlalchemy.engine import Result
from sqlalchemy.ext.asyncio import AsyncSession
from sqlalchemy.exc import SQLAlchemyError
from fastapi import HTTPException

from app.models.user import User
from app.schemas.user import UserCreate, UserUpdate
from app.core.security import get_password_hash

# ORM 개념 설명:
# ORM(Object Relational Mapping)은 객체 지향 프로그래밍 언어를 사용하여 데이터베이스를 조작하는 기술입니다.
# SQLAlchemy는 Python의 ORM 라이브러리로, 데이터베이스 테이블과 Python 클래스를 매핑하여 데이터베이스 작업을 쉽게 할 수 있게 해줍니다.

# 모든 사용자를 데이터베이스에서 조회하는 함수
async def get_users(db: AsyncSession): 
   
    result: Result = await db.execute(text("select id,username,email from users")) 
    # result: Result = db.execute(select(User)) 
    await db.close()
    # results_as_dict = result.fetchall()
    print(result)
    return result

# 특정 사용자를 ID로 조회하는 함수
async def get_user(db: AsyncSession, user_id: int):

    result: Result = await db.execute(text("select id,username,email from users where id ='"+ user_id +"'")) 
    # result: Result = db.execute(select(User)) 
    await db.close()
    # results_as_dict = result.fetchall()
    print(result)
    return result



# 특정 사용자를 이메일로 조회하는 함수
def get_user_by_email(db: AsyncSession, email: str):
    # db.query(User): User 테이블에서 쿼리 생성
    # .filter(User.email == email): 특정 조건(이메일)으로 필터링
    # .first(): 조건에 맞는 첫 번째 레코드 반환
    return db.query(User).filter(User.email == email).first()

# 새로운 사용자를 생성하는 함수
async def create_user(db: AsyncSession, user: UserCreate):
    # print(user)
    # 비밀번호 해시화 (암호화)
    hashed_password = get_password_hash(user.password)
    print(hashed_password)
    
    query_in = text("insert into users (username,email,hashed_password,is_active)  \
                values (:username,:email,:hashed_password,1)")
    # t = {user.username,user.email, hashed_password}
    t = {"username":user.username, "email":user.email, "hashed_password":hashed_password}
    
    try:
        await db.execute(query_in,t)
        # await db.commit()
    except SQLAlchemyError  as e:
        await db.rollback()  # 에러 발생 시 롤백 
        await db.close()  # 세션 닫기
        print(f"Error: {e}")
    finally:
        await db.commit()
        await db.close()  # 세션 닫기

    result: Result = await db.execute(text("select id,username,email from users where email ='"+ user.email +"'")) 
    # result: Result = db.execute(select(User)) 
    await db.close()
    # results_as_dict = result.fetchall()
    print(result)
    return result

# 기존 사용자를 업데이트하는 함수
async def update_user(db: AsyncSession, user_id: int, user: UserUpdate):
    print(user_id)
    hashed_password = get_password_hash(user.password)
    query_in = text("update users set username='"+user.username+"',email='"+user.email+"',hashed_password='"+hashed_password+"' where id = "+str(user_id))
    print(query_in)
    try:
        await db.execute(query_in)
        # await db.commit()
    except SQLAlchemyError  as e:
        await db.rollback()  # 에러 발생 시 롤백 
        await db.close()  # 세션 닫기
        print(f"Error: {e}")
    finally:
        await db.commit()
        await db.close()  # 세션 닫기
    result = await get_users(db)


    # # ID로 기존 사용자 조회
    # db_user = db.query(User).filter(User.id == user_id).first()
    # # 사용자 정보 업데이트
    # db_user.username = user.username
    # db_user.email = user.email
    # db_user.hashed_password = get_password_hash(user.password)
    # # 변경 사항 커밋 (저장)
    # db.commit()
    # print(db_user)
    # # 업데이트된 사용자 반환
    return result

# 사용자를 삭제하는 함수
async def delete_user(db: AsyncSession, user_id: int):
    # # ID로 사용자 조회
    # db_user = db.query(User).filter(User.id == user_id).first()
    # # 데이터베이스에서 사용자 삭제
    # db.delete(db_user)
    # # 변경 사항 커밋 (저장)
    # db.commit()
    # # 삭제된 사용자 반환
    # return db_user
    item = await db.get(User, user_id)
    if not item:
        raise HTTPException(status_code=404, detail="Item not found")
    
    # 2. Delete and commit
    await db.delete(item)
    await db.commit()
    await db.close()
    return {"ok": True}
